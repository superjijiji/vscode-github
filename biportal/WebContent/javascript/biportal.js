 ...
 */
try {
	var biportal_context = location.pathname.substring(0,location.pathname.lastIndexOf('/'));
	biportal_context = biportal_context.split("/action",2)[0];
	...
	BIReports.prototype.empty = function(){
		if(this.reports){
...
//			tables += '</tr>';
	...
	BIReport.prototype.openReport = function(menu_id){
		...
		if(rpt_type=='cognos'){
			if(this.report.objectClass=='URL'){
				...
				//add by leo calculate report access count for ranking
				if(this.report.searchPath.indexOf('%2F') == -1){
					jQuery
					.ajax({
						type : "get",
						url : biportal_context+"/action/portal/ranking/calculateReportAccess?searchPath="+ encode(this.report.searchPath),
						success : function(data) {
					...
					.ajax({
						type : "get",
						url : biportal_context+"/action/portal/ranking/calculateReportAccess?searchPath="+ this.report.searchPath,
						success : function(data) {
		...
		if(rpt_type=='IR'){
			var timeid = (new Date()).valueOf();
			var url = biportal_context+"/action/portal/report/openIRReport?timeid="+timeid;
	...
	BIReport.prototype.forwardToCognos = function(menu_id,action_param){
		...
		//add by leo for calculate report access count in IT07
		if(operations[0]=='run_with_CV' || operations[0]=='run_with_option'){
	...
	BIReport.prototype.mailReport = function(menu_id){
		...
		if(this.report.reportType=='IR'){
			var timeid = (new Date()).valueOf();
			var url = biportal_context+"/action/portal/report/mailIRReport?timeid="+timeid;
			var newWin = window.open('', '_blank', 'resizable=yes,toolbar=no,menubar=no,scrollbars=yes');
			jQuery.ajax({
				type : "POST",
				url : url,
	...
	ActionButton.prototype.toString = function() {
		...
		//add by leo for report rank
		if(this.report.report.reportType == 'cognos'){
			if(this.report.report.searchPath.indexOf("%") == -1){
				str_button +='<a class="ibm-star-none-link" style="margin-bottom: 1px;margin-bottom: 1px;padding-bottom: 1px;padding-top: 1px;" href="#" id=\'' + this.report.report.domain_Key +"_"+ this.report.report.rptObjID + '_rank_id\' title="rank this report" onclick="popupDialogOne(\''+encode(this.report.report.searchPath)+'\',\'' +this.report.report.domain_Key +'\'); return false;" >rank this report</a>';
			}else{
				str_button +='<a class="ibm-star-none-link" style="margin-bottom: 1px;margin-bottom: 1px;padding-bottom: 1px;padding-top: 1px;" href="#" id=\'' + this.report.report.domain_Key +"_"+ this.report.report.rptObjID + '_rank_id\' title="rank this report" onclick="popupDialogOne(\''+this.report.report.searchPath+'\',\'' +this.report.report.domain_Key +'\'); return false;">rank this report</a>';
			}
...
//add by leo for rank
function popupDialogOne(searchPath,domainKey){
//		IBMCore.common.widget.overlay.hideAllOverlays();
//		var myOverlay = IBMCore.common.widget.overlay.createOverlay({
//	        contentHtml: '<div style="height: 800px;width:800px" data-widget="overlay" >' + domainKey + '|' + decodeURIComponent(searchPath) + '</div>',
...
//	    myOverlay.show();
		...
		var timeid = (new Date()).valueOf();
		var url = biportal_context+"/action/portal/rank/getReportRankingPage?search_path="+searchPath;
